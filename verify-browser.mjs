/**
 * Headless-browser smoke test for the ModelTester plugin via Chrome DevTools
 * Protocol. Launches Edge (already running with --remote-debugging-port),
 * opens the dsh web app, waits for React to mount, and asserts the ModelTester
...
async function main() {
  ...
  const isPluginNoise = (text) => hostNoise.test(text) && !/modeltester/i.test(text)
  cdp.on((msg) => {
  ...
  while (Date.now() < deadline) {
    ...
    if (/ModelTester|推理轨迹/.test(body)) { foundPanel = true; break }
  }
