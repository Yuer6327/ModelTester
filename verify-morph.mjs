/**
 * Morph-interaction smoke test: drives the ModelTester panel in a headless Edge
...
async function main() {
  ...
  cdp.handler = (m) => {
    if (m.method === 'Runtime.exceptionThrown') {
      ...
      if (hostNoise.test(text) && !/modeltester|ModelTester/i.test(text)) return
      errors.push(text)
    }
  ...
  // Wait for the panel root (has ModelTester text).
  let box = null
  const deadline = Date.now() + 45000
  while (Date.now() < deadline) {
    await sleep(1200)
    box = await evalJs(`(() => {
      const pill = document.querySelector('button[title*="ModelTester"]');
  ...
  const shotDir = join(tmpdir(), 'modeltester-verify')
  mkdirSync(shotDir, { recursive: true })
  await cdp.send('Page.captureScreenshot', { format: 'png' }).then(r => { writeFileSync(join(shotDir, 'modeltester-pill.png'), Buffer.from(r.data, 'base64')) })

  // Expand: click the pill.
  await evalJs(`(() => {
    const p = document.querySelector('button[title*="ModelTester"]');
    if (p) p.click();
    return !!p;
  })()`)
  await sleep(800)
  ...
  await cdp.send('Page.captureScreenshot', { format: 'png' }).then(r => { writeFileSync(join(shotDir, 'modeltester-card.png'), Buffer.from(r.data, 'base64')) })
