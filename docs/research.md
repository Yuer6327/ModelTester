...
ModelTester's three categories are not guesses — they are the lexical fingerprints
...
(block count and character length). ModelTester mirrors that exactly — the
...
reasoning blocks, ModelTester also tracks the text totals as a **diagnostic**:
